...
namespace Illusion.Extensions
{
  public static class StringExtensions
  {
    ...
    public static Color GetColor(this string self)
    {
      ...
      return colorCheck.HasValue ? colorCheck.Value : Color.clear;
    }
    ...
    public static Color? GetColorCheck(this string self)
    {
      ...
      if (strArray.Length >= 3)
      {
        ...
        int num2 = index1 + 1;
        Color color;
        float.TryParse(array1.SafeGet<string>(index1), out color.r);
        string[] array2 = strArray;
        int index2 = num2;
        int num3 = index2 + 1;
        float.TryParse(array2.SafeGet<string>(index2), out color.g);
        string[] array3 = strArray;
        int index3 = num3;
        int num4 = index3 + 1;
        float.TryParse(array3.SafeGet<string>(index3), out color.b);
        string[] array4 = strArray;
        int index4 = num4;
        int num5 = index4 + 1;
        if (!float.TryParse(array4.SafeGet<string>(index4), out color.a))
          color.a = 1f;
        for (int index5 = 0; index5 < num5; ++index5)
        {
          if ((double) color[index5] > 1.0)
            color[index5] = Mathf.InverseLerp(0.0f, (float) byte.MaxValue, color[index5]);
        }
        return new Color?(color);
      }
      Color color1;
      return ColorUtility.TryParseHtmlString(self, out color1) ? new Color?(color1) : new Color?();
    }
